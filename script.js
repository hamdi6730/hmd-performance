...
formPlaque?.addEventListener("submit", (e) => {
  ...
  showLoaderAndExecute(async () => {
    try {
      const response = await fetch(`https://api-plaque-immatriculation-siv.p.rapidapi.com/get-vehicule-info?token=TokenDemoRapidapi&host_name=https%3A%2F%2Fapiplaqueimmatriculation.com&immatriculation=${plateValue}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'x-rapidapi-host': 'api-plaque-immatriculation-siv.p.rapidapi.com',
          'x-rapidapi-key': '7f91d34e65msh2d69ec6c8975bf6p147d95jsn455e3b396047'
        }
      });
      if (response.ok) {
        const result = await response.json();
        // The API returns result.code_erreur === 200 on success. It does not return a result.status property.
        if (result && result.code_erreur === 200 && result.data && !result.data.erreur) {
          ...
          // Fuel detection based on type_moteur or energieNGC
          const fuelType = (apiCar.type_moteur || apiCar.energieNGC || "").toLowerCase();
          const isDiesel = fuelType.includes("diesel") || fuelType.includes("gazole");
          
          // Horsepower from puisFiscReelCH (ex: "131 CH") or puisFiscReelKW
          let puissanceCh = 100;
          if (apiCar.puisFiscReelCH) {
            puissanceCh = parseInt(apiCar.puisFiscReelCH) || puissanceCh;
          } else if (apiCar.puisFiscReelKW) {
            puissanceCh = Math.round(parseInt(apiCar.puisFiscReelKW) * 1.36) || puissanceCh;
          }

          // Turbo detection
          const searchStr = `${apiCar.version || ""} ${apiCar.modele_en || ""} ${apiCar.sra_commercial || ""}`.toLowerCase();
          const isTurbo = isDiesel || 
                          searchStr.includes("tce") || 
                          searchStr.includes("tsi") || 
                          searchStr.includes("tfsi") || 
                          searchStr.includes("tgdi") || 
                          searchStr.includes("t-gdi") || 
                          searchStr.includes("ecoboost") || 
                          searchStr.includes("boosterjet") || 
                          searchStr.includes("turbo") || 
                          searchStr.includes("dci") || 
                          searchStr.includes("tdi") || 
                          searchStr.includes("hdi") || 
                          searchStr.includes("bluehdi") || 
                          searchStr.includes("cdti") || 
                          searchStr.includes("crdi") || 
                          searchStr.includes("d-4d") || 
                          searchStr.includes("d4d") || 
                          searchStr.includes("jtd") || 
                          searchStr.includes("tdci") || 
                          searchStr.includes("mjtd");
          
          // Torque estimation based on fuel, turbo, and HP
          let couple = Math.round(puissanceCh * 1.5);
          if (isDiesel) {
            couple = Math.round(puissanceCh * 2.25);
          } else if (isTurbo) {
            couple = Math.round(puissanceCh * 1.6);
          } else {
            couple = Math.round(puissanceCh * 1.1);
          }

          // Build a readable engine string for display
          let engineDesc = apiCar.version || "";
          if (apiCar.puisFiscReelCH) {
            const chStr = apiCar.puisFiscReelCH.includes("CH") ? apiCar.puisFiscReelCH : `${apiCar.puisFiscReelCH} CH`;
            engineDesc += ` ${chStr}`;
          }
          if (apiCar.type_moteur) {
            engineDesc += ` (${apiCar.type_moteur})`;
          }
          engineDesc = engineDesc.trim().replace(/\s+/g, ' ');

          const vehicle = {
            id: "dynamic_siv",
            brand: brand,
            model: model,
            engine: engineDesc || "Moteur",
      ...
      console.warn("Real SIV API lookup failed. Using deterministic local simulation:", err);
    }
